    ...
    ProvideWindowInsets {
        JetsnackTheme {
            Crossfade(navigator.current) { destination ->
                when (destination) {
                    Destination.Home -> Home(actions.selectVideoCategory, actions.selectGankGirl)
                    is Destination.VideoCategoryDetail -> SnackDetail(
                        snackId = destination.snackId,
                        upPress = actions.upPress
                    )
                    is Destination.GankDetail -> GankDetail(
                        gankStr = destination.gank,
                        upPress = actions.upPress
                    )
                }
            }
